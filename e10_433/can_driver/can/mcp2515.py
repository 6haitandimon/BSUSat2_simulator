...
from device_controller.device_config import InternalSPIConfig
...
class CAN:
    def __init__(self, SPI: Any) -> None:
        ...
        self.can_rx_tsf = ThreadSafeFlag()
        self.can_irq = Pin(InternalSPIConfig.CAN_PICO_IRQ_PIN, Pin.IN)

    def enable_interrupt(self):
        self.can_irq.irq(trigger=Pin.IRQ_FALLING, handler=self.can_rx_irq_callback)
    ...
    def reset(self) -> int:
        ...
        # meet filter criteria. RXF0 is applied for RXB0, RXF1 is applied for RXB
        self.modifyRegister(
            REGISTER.MCP_RXB0CTRL,
    ...
    def can_rx_irq_callback(self, pin: Pin) -> None:
        self.can_rx_tsf.set()
    ...
    def sendMessage_(self, frame: Any) -> int:
        ...
        for i in range(N_TXBUFFERS):
            ...
            if (ctrlval & TXBnCTRL.TXB_TXREQ) == 0:
                # Clear ABTF MLOA TXERR in TX_CTRL REG
                self.modifyRegister(txbuf.CTRL, TXBnCTRL.TXB_ABTF | TXBnCTRL.TXB_MLOA | TXBnCTRL.TXB_TXERR, 0)
                ctrlval = self.readRegister(txbuf.CTRL)   
